...
$overrideStmt = $pdo->prepare('SELECT * FROM listing_overrides WHERE contract_id = :id LIMIT 1');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $save = $pdo->prepare('INSERT INTO listing_overrides
    ...
    admin_log_activity($pdo, (int) $adminUser['id'], 'listing_override_updated', 'listing', $id, [
        'is_hidden' => $isHidden,
        'is_featured' => $isFeatured,
        'quality_flag' => $qualityFlag,
    ]);

    header('Location: listing_edit.php?id=' . $id . '&saved=1');
    exit;
}

if (request_int('saved', 0) === 1) {
    $message = 'Listing override saved.';
    $overrideStmt->execute(['id' => $id]);
    $override = $overrideStmt->fetch() ?: [];
}

$adminTitle = 'Listing Edit';
include __DIR__ . '/includes/admin_header.php';
?>
<h1>Listing #<?php echo (int) $id; ?></h1>
<?php if ($message): ?><p class="badge badge-ok"><?php echo e($message); ?></p><?php endif; ?>
<section class="card">
  <p><strong>Raw Title:</strong> <?php echo e((string) $listing['title']); ?></p>
  <p><strong>Agency:</strong> <?php echo e(display_field_value('agency', $listing['agency_name'] ?? null)); ?></p>
  <p><strong>Category:</strong> <?php echo e(display_field_value('category', $listing['category_name'] ?? null)); ?></p>
  <p><strong>Source Type:</strong> <?php echo e((string) $listing['source_type']); ?></p>
</section>
...
<section class="card">
  <form method="post" class="admin-form-block">
    ...
    <a class="btn btn-secondary" href="listings.php">Back to Listings</a>
  </form>
</section>
