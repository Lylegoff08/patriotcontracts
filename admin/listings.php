    ...
    if ($contractId > 0) {
        try {
            if ($action === 'toggle_hidden') {
                $stmt = $pdo->prepare('INSERT INTO listing_overrides (contract_id, is_hidden, updated_by, created_at, updated_at)
                ...
                admin_log_activity($pdo, (int) $adminUser['id'], 'listing_toggle_hidden', 'listing', $contractId, []);
                $message = 'Listing visibility updated.';
            } elseif ($action === 'toggle_featured') {
                $stmt = $pdo->prepare('INSERT INTO listing_overrides (contract_id, is_featured, updated_by, created_at, updated_at)
                    VALUES (:contract_id, 1, :updated_by, NOW(), NOW())
                    ON DUPLICATE KEY UPDATE is_featured = IF(is_featured = 1, 0, 1), updated_by = VALUES(updated_by), updated_at = NOW()');
                $stmt->execute(['contract_id' => $contractId, 'updated_by' => (int) $adminUser['id']]);
                admin_log_activity($pdo, (int) $adminUser['id'], 'listing_toggle_featured', 'listing', $contractId, []);
                $message = 'Listing featured flag updated.';
            } elseif ($action === 'save_note') {
                $note = trim(request_str('internal_note'));
                $stmt = $pdo->prepare('INSERT INTO listing_overrides (contract_id, internal_notes, updated_by, created_at, updated_at)
                ...
                admin_log_activity($pdo, (int) $adminUser['id'], 'listing_note_updated', 'listing', $contractId, ['length' => strlen($note)]);
...
$sql = 'SELECT cc.id,
    COALESCE(NULLIF(lo.display_title, ""), cc.title) AS title,
    a.name AS agency_name,
    s.name AS source_name,
    cc.posted_date,
    COALESCE(lo.is_hidden, 0) AS is_hidden,
    COALESCE(lo.is_featured, 0) AS is_featured,
    COALESCE(NULLIF(lo.quality_flag, ""), "") AS quality_flag,
    COALESCE(lo.internal_notes, "") AS internal_notes
    FROM contracts_clean cc
...
<section class="card">
  ...
  <p class="muted">Total <?php echo number_format($totalRows); ?> listings. Page <?php echo (int) $page; ?> of <?php echo (int) $totalPages; ?>.</p>
</section>
        ...
        <tr>
          ...
          <td>
            <div class="admin-actions">
              ...
              <a class="btn btn-secondary" href="listing_edit.php?id=<?php echo (int) $row['id']; ?>">Quick Edit</a>
